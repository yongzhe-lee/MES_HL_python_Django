...
class PMService():
    ...
    def findAllPm(self, keyword, equDept, equLoc, pmDept, pmType, applyYn, cycleType, sDay, eDay, isMyTask, isLegal):
        items = []
        dic_param = {'keyword': keyword,'equDept': equDept,'equLoc': equLoc,'pmDept': pmDept,'pmType': pmType,'applyYn': applyYn,'cycleType': cycleType,'sDay': sDay,'eDay': eDay,'isMyTask': isMyTask,'isLegal': isLegal}
		...
		select t.pm_pk
			...
			, pt.code_nm as pm_type_nm
			, cm_fn_user_nm(pmu.user_nm, pmu.del_yn) as pm_user_nm
            , cm_fn_user_nm(wou.user_nm, wou.del_yn) as wo_user_nm
            , t.work_charger_pk
            , t.dept_pk
        ...
        if deptPk:
            sql += '''
				...
				AND (
					d.dept_pk = %(deptPk)s
					OR
					d.dept_pk In (select dept_pk from cm_v_dept_path where 1 = path_info_pk)
				)
               '''
